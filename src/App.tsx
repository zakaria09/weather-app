...
function App() {
  ...
  const renderForecast = () => {
    if (error) return <div className='px-8 border-4 border-red-400 border-solid p-8 rounded-lg bg-red-50'><p className='text-xl font-semibold'>Oops somethin went wrong!</p></div>
    else if (isLoading) return (
      <div className="px-8">
        <div className="h-12 w-12 animate-spin rounded-full border-b-4 border-current" />
      </div>
    );
    else return <div className="w-full flex flex-col md:flex-row">
      ...
      <div className="basis-9/12 px-6">
        {currentForecast ? <WeeklyForecast forecast={currentForecast} /> : null}
      </div>
    </div>;
  }
